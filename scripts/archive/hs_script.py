...
if msi_test is True:
    # be sure to set keyrings
    msi_run_id = 1
    dir_base = '/panfs/roc/groups/5/yangc1/public/hs_process'
    # globus_client_id = keyring.get_password('globus_client_id', 'nigo0024')
    # transfer_at = keyring.get_password('globus_transfer_access_token', 'nigo0024')
    # transfer_rt = keyring.get_password('globus_transfer_refresh_token', 'nigo0024')
    globus_client_id = '684eb60a-9c5e-48af-929d-0880fd829173'
    transfer_at = 'Aggwn2QG9WQb7v5QmBe1KbgBDmBKd3mEwXx3wYQxdQDg80xnoXSJCx57ywM1qpV6NNgPJOPWgdMYYJT1Q5Bg2flGOG'
    transfer_rt = 'AgeQ96Dj433owed0EK799GkB4jNbl5xkWDrMjjqgQzVbNoPBKHqUM1bGngqy5DB0elpkv637z3qM8pMy27BMJdxyeE5n'
...
if __name__ == "__main__":  # required on Windows, so just do on all..
    ...
    for y_idx, y_label in enumerate(y_label_list):
        ...
        for extra_feats, extra_name in zip(extra_feats_list, extra_feats_names):
            ...
            df_tune_all_list = execute_tuning_pp(
                ...
                key, df_train, n_splits, n_repeats, df_tune_all_list)
            ...
            for idx, df_score in enumerate(df_score_list):
                ...
                #     array = [(folder_list_test, dir_out_list_test, feat_n, df_pred_list,
                #               df_score_list, model_list, x_label, y_label1,
                #               y_label, units, True, legend_cols)
                #               for feat_n in df_score[pd.notnull(df_score['feats'])]['feat_n']]
                #     # executor.map(execute_tuning, *zip(*array))
                #     executor.map(plot_pred_figure_pp, *zip(*array))

                # itr = it.repeat
                # # df_score = pd.read_csv(r'G:\BBE\AGROBOT\Shared Work\hs_process_results\results\msi_0_results\msi_0_033\biomass_kgha\spectral\testing\msi_0_033_biomass_kgha_test-scores-lasso.csv')
                # feat_n_series = df_score[pd.notnull(df_score['feats'])]['feat_n']
                # # for feat_n in feat_n_series:
                # #     print(feat_n)
                # with ProcessPoolExecutor() as executor:
                #     executor.map(
                #         plot_pred_figure_pp, itr(folder_list_test),
                #         itr(dir_out_list_test), feat_n_series, itr(df_pred_list),
                #         itr(df_score_list), itr(model_list), itr(x_label),
                #         itr(y_label1), itr(y_label), itr(units), itr(True),
                #         itr(legend_cols))
    ...
    if msi_run_id > 0:  # be sure msi_id is adjusted when actually running on msi
        ...
        globus_transfer(
            dir_source_data, dir_dest_data, transfer_rt, client, transfer_at,
            label=label_base + '-data', delete=True)

        dir_source_results, dir_dest_results = get_globus_results_dir(
            dir_base, msi_run_id, row)
        globus_transfer(
            dir_source_results, dir_dest_results, transfer_rt, client,
            transfer_at, label=label_base + '-results', delete=True)
