...
if __name__ == "__main__":  # required on Windows, so just do on all..
    ...
    time_dict = time_setup_training(dir_results, y_label_list, extra_feats_names, msi_run_id)
    ...
    for idx_grid, row in df_grid.iterrows():
        ...
        for y_idx, y_label in enumerate(y_label_list):
            ...
            for extra_feats, extra_name in zip(extra_feats_list, extra_feats_names):
                ...
                # logspace_list = filter_logspace_list(
                #     logspace_list_full, X1, y1, max_iter, random_seed)
                ...
                #         max_iter, random_seed, key, df_train, n_splits, n_repeats, n_jobs)
