...
#SBATCH -o ./reports_procimg_array/R-%A-%a-%x.out
#SBATCH -e ./reports_procimg_array/R-%A-%a-%x.err
...
export msi_run_id start n CLIENT_ID TT TRT

conda activate sip_run_2

python sip/process_img.py --n_jobs $SLURM_NTASKS --msi_run_id $msi_run_id --idx_min $start+0 --idx_max $start+$n
wait
